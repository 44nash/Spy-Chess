...
const getStatusMessage = ({ spectator, curPlr, winner, finished, players }) => {
  if (finished) {
    if (winner != null) {
      if (spectator) {
...
function Game() {
  ...
  useEffect(() => {
    ...
    const setChessBoardWidthWithContainerWidth = () => {
      setChessBoardWidth(
        Math.min(CHESS_WIDTH_MAX_PX, containerRef.current.offsetWidth) -
          CHESS_TOTAL_PADDING_PX
      );
    };
  ...
  const {
    players = [],
    state: { fen, plrIdToColor, winner, lastMovedSquare } = {},
    finished,
  } = roomState;
        ...
        >
          <Stack spacing={1}>
            ...
            <Typography color="text.primary">
              {plrIdToUsername[
                colorToPlrId[boardOrientation === "white" ? "black" : "white"]
              ] ?? ""}
            </Typography>
            <Box>
              <Board
            ...
            <Typography color="text.primary">
              {plrIdToUsername[colorToPlrId[boardOrientation]] ?? ""}
            </Typography>
          </Stack>
        </Stack>
      </SnackbarProvider>
